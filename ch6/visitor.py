...
vistor_*/.py subclasses use cases; frameworks should generally use__X pseudo
private names, but all names here are exported for use in subclasses and clients;
redefine reset to support multiple independed walks that require subclass updates;
"""
...
class FileVisitor:
    """
    Visit all nondirectory files below startDir (default'.');
    override visit* methods to provide custom file/dir handlers;
    context arg/attribute is option subclass-specific state;
    trace witch: 0 is silent, 1 is directories, 2 adds files
    """
...
class SearchVisitor(FileVisitor):
    ...
    also redefine candidate to use mimetypes for text content)
    """
