...
enough to use os.environ['PYTHONPATH']: this is a subest of sys.path.
"""
...
visited = {}
allsizes = []
for srcdir in sys.path:  # every directory in sys.path
    for (thisDir, subsHere, filesHere) in os.walk(srcdir):
        if trace > 0: print(thisDir)
        thisDir = os.path.normpath(thisDir)
        fixcase = os.path.normcase(thisDir)
        if fixcase in visited:
            continue  # skips the rest of the code, goes into new loop
        else:
